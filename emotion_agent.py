...
# Function: Emotion & Sentiment Detection
def detect_emotion_sentiment(text: str) -> dict:
    ...
    for emo, keywords in emotion.items():
...
def generate_creative_response(text: str) -> str:
    ...
    return f"✨ Here's a creative response to your input: {text[::-1]}"

# LangChain Tool
def analyze_text(text: str) -> str:
    result = detect_emotion_sentiment(text)
    return f"""Sentiment: {result['sentiment']}
Dominant Emotion: {result['dominant_emotion']}
Emotion Scores: {result['emotion_scores']}"""

emotion_tool = Tool(
    name="EmotionSentimentAnalyzer",
    func=analyze_text,
    description="Analyzes text to detect emotion and sentiment"
)
